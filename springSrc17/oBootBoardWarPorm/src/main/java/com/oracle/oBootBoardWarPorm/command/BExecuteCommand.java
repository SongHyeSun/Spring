package com.oracle.oBootBoardWarPorm.command;
...
//		  2) request 이용 ->  bid  추출
		HttpServletRequest request = (HttpServletRequest) map.get("request");
		int strbId = Integer.parseInt(request.getParameter("bId"));
		System.out.println("bReplyViewCmd strbId->"+strbId);
//		  3) dao  instance 선언
		BDto bDto = jdbcDao.reply_view(strbId);
